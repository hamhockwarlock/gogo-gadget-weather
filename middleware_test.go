...
func TestApiKeyNotPassed(t *testing.T) {
  ...
  if errorMessage := parsed.Error; errorMessage != http.StatusText(http.StatusUnauthorized) {
    t.Errorf("handler returned the wrong error: got %s want %s", errorMessage, http.StatusText(http.StatusUnauthorized))
  }


  if message := parsed.Message; message != apiKeyRequiredMessage {
    t.Errorf("handler returned the wrong message: got %s want %s", message, apiKeyRequiredMessage)
  }
}
