...
func TestHandleNotFound(t *testing.T) {
  ...
  if status := rr.Code; status != http.StatusNotFound {
    t.Errorf("handler returned the wrong status code: got %v want %v", status, http.StatusNotFound)
  }
  ...
  if errorMessage := parsed.Error; errorMessage != http.StatusText(http.StatusNotFound) {
    t.Errorf("handler returned the wrong error: got %s want %s", errorMessage, http.StatusText(http.StatusNotFound))
  }
...
func TestHandleMethodNotAllowed(t *testing.T) {
  ...
  if status := rr.Code; status != http.StatusMethodNotAllowed {
    t.Errorf("handler returned the wrong status code: got %v want %v", status, http.StatusMethodNotAllowed)
  }
